    ...
    <div className="relative flex-1 flex flex-col w-full h-full bg-stone-50 dark:bg-stone-900">
      ...
      <main className="flex-1 overflow-y-auto p-6 pt-0 pb-32 space-y-4">
        {loading ? (
          ...
          goals.map((goal) => (
            <div
              key={goal.id}
              className="p-5 bg-white dark:bg-stone-800 rounded-3xl shadow-sm border border-stone-100 dark:border-stone-700 relative overflow-hidden"
            >
              {goal.is_automated && (
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h2 className="font-bold text-stone-900 dark:text-white text-lg">{goal.title}</h2>
                  {goal.is_automated && (
                    <p className="text-xs text-stone-400">Vinculado a: {goal.linked_account_name}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-medium">
                  ...
                  <span className="text-stone-900 dark:text-stone-200">
                    R$ {goal.current_amount.toLocaleString('pt-BR')} <span className="text-stone-400">/ {goal.target_amount.toLocaleString('pt-BR')}</span>
                  </span>
                </div>
      ...
      <div className="absolute bottom-8 right-6 flex flex-col items-center gap-2">
        ...
        <button
          ...
          className="w-16 h-16 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-full flex items-center justify-center shadow-xl hover:scale-105 transition-transform"
        >
          <Plus size={32} />
        </button>
      </div>
